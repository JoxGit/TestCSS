...
document.querySelector('#primary > input').oninput = (event) => {
    ...
    document.body.style.setProperty('--matter-primary-rgb', `${Math.round(rgb[0] * 255)}, ${Math.round(rgb[1] * 255)}, ${Math.round(rgb[2] * 255)}`);
};

document.querySelector('#on-primary > input').oninput = (event) => {
    const value = event.target.value;
    document.body.style.setProperty('--matter-onprimary-rgb', `${value}, ${value}, ${value}`);
};

document.querySelector('#surface > input').oninput = (event) => {
    const value = event.target.value;
    document.body.style.setProperty('--matter-surface-rgb', `${value}, ${value}, ${value}`);
};

document.querySelector('#on-surface > input').oninput = (event) => {
    const value = event.target.value;
    document.body.style.setProperty('--matter-onsurface-rgb', `${value}, ${value}, ${value}`);
};
